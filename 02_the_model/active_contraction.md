...
Active tension is stored by anatomical region, so the LV free wall, septum, and RV free wall can in principle be assigned different peak amplitudes. In the simulations reported here, however, the peak active tension is held fixed at 100 kPa in all three regions, a value in the range typically used for cardiac active-stress models [ADD CITATION]. This choice keeps the contractile drive from becoming an additional region-specific tuning parameter: the different LV and RV pressure regimes then arise from geometry, boundary conditions, material response, and circulatory loading rather than from prescribing a lower RV myocardial tension.
