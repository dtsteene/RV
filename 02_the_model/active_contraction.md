...
This section defines the scalar active tension $T_a(t)$. Biologically, $T_a$ represents the force-generating action of the sarcomeres, the molecular motors embedded within each muscle fibre. Mechanically, it is a prescribed fibre-aligned tension in kPa that turns the passive finite-strain material into an actively contracting myocardium.
...
The cardiac clock is fixed at 75 beats per minute (RR interval 0.8 s). Because the finite-element mesh represents the end-diastolic configuration, ventricular activation is aligned with the start of the simulated beat by setting $t_C=0$. The contraction and relaxation durations are held fixed at $T_C=0.25$ s and $T_R=0.40$ s, leaving the rest of the cycle for diastolic filling. These values are part of the fixed mechanics model, not parameters retuned across the pressure sweep.

## Spatial Assignment of Active Tension

Active tension is stored by anatomical region, so the LV free wall, septum, and RV free wall can in principle be assigned different peak amplitudes. In the simulations reported here, however, the peak active tension is held fixed at 100 kPa in all three regions. This is a modelling-scale choice of the same order as active-tension amplitudes used in related coupled cardiac active-stress models, rather than a patient-specific contractility estimate {cite}`regazzoni2022cardiac,piersanti2022closed`. This choice keeps the contractile drive from becoming an additional region-specific tuning parameter: the different LV and RV pressure regimes then arise from geometry, boundary conditions, material response, and circulatory loading rather than from prescribing a lower RV myocardial tension.
