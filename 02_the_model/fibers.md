...
The right ventricular free wall has a different architecture. The RV wall is much thinner and wraps around the outside of the LV in a crescent, so its fibres are shallower with less transmural rotation. The model uses prescribed LDRB angles $\alpha_\text{endo} = +90^\circ$ and $\alpha_\text{epi} = -25^\circ$, so the RV fibre points almost straight along the long axis at the endocardium and tilts only modestly the other way at the epicardium. This separate RV rule follows biventricular rule-based fibre models that treat the RV and septum differently from the LV free wall {cite}`doste2019rulebased`.

To apply the two angle conventions, every cell must first be classified as LV-side or RV-side. {numref}`fig-lv-rv-partition` shows this cell-level partition, defined by the sign of $\phi_\text{lv}-\phi_\text{rv}$ — two further Laplace solutions with Dirichlet value 1 on the LV and RV endocardial surfaces respectively. The partition is used only inside the LDRB step to pick which $\alpha$ rule applies to each cell. It is distinct from the geometric LV/RV/septum partition in {numref}`fig-mesh-regions`, which is used everywhere else in the thesis.
