...
This pressure mismatch does not compromise the volume coupling — the volumes match by construction at every time step — but it has two significant implications. First, the 0D pressure that enters the valve flow equations and the Windkessel circuits is not the same as the pressure that the myocardium is actually generating. The hemodynamic state computed by the 0D model (arterial pressures, valve timing, flow waveforms) is therefore based on approximate chamber pressures. Second, and more relevant for this thesis, the 0D pressure cannot be used as a proxy for the actual endocardial loading when computing boundary work or evaluating pressure-strain proxies. For energy conservation checks and for the pressure-strain work proxies evaluated in the results chapter, the Lagrange multiplier pressure from the 3D solver is the physically correct quantity. The 0D pressure should be understood as a driving signal that produces the correct volume dynamics, not as a measurement of the true cavity pressure.
