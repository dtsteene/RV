...
The remainder of the chapter follows the pipeline. {ref}`sec-geometry-anatomical-model` defines the mesh and anatomical tags. {ref}`sec-fibers` explains how the local fibre-sheet-normal frame is assigned. {ref}`sec-3d-mechanics` gives the inverse-unloaded reference configuration, finite-strain kinematics, passive Holzapfel-Ogden material law, active-stress contribution including the Blanco activation waveform, equilibrium problem, and boundary/support conditions. {ref}`sec-0d-circulation` then gives the standalone 0D model and the 3D--0D coupling.
