...
The results are easiest to understand if two questions are kept separate. First: does the pressure-strain proxy get the regional work differences right? Second: when the circulation is changed while geometry is held fixed, does the proxy rank loading cases in the same order as the finite-element stress-strain work density? These are both useful questions, but they are not the same. A proxy can rank a pressure sweep well and still give the wrong regional balance in a single patient.
...
Unless stated otherwise, the results below use the high-resolution capped-reference pressure-loading sweep described in {ref}`chap-calibration`. This sweep runs at 5 mm characteristic mesh length. All 16 cases completed per-case canonical postprocessing, and all pressure and per-cell work arrays passed finite-value checks. In that sweep the biventricular atlas target, material parameters, boundary conditions, activation waveform, and end-diastolic mesh target are held fixed, while the circulation parameters are changed to raise RV pressure over a controlled range. The RV end-diastolic pressure used only for inverse unloading is capped at 5 mmHg, as motivated in {ref}`chap-calibration`. Case names such as `sPAP55` are nominal calibration labels; pressure axes, correlations, and proxy calculations use the achieved pressures and volumes from the coupled finite-element/circulation simulations. All pressure-strain proxies use the solver cavity pressures returned by the volume-constrained mechanics problem, not the standalone 0D elastance pressures.

The numbers in this chapter are produced under the per-case canonical scheme: each capped case is independently tetrahedralised, tagged on its own reference end-diastolic mesh, and integrated. A reference-tag rerun is in progress, in which a single fixed reference mesh and a single set of region tags are used across all sweep cases so that only the circulation changes from one case to the next ({ref}`sec-reference-tag-postprocessing`). The reference-tag protocol is the more defensible cross-case design and will replace the tables in this chapter once the rerun completes; the conclusions reported here are not expected to change qualitatively, but absolute work-density magnitudes and the small case-to-case mesh differences will be tightened.
...
Across the primary sweep, the LV adjacent-pressure longitudinal-strain index correlated strongly with LV free-wall stress-strain work density ($r=0.928$). Peak LV longitudinal shortening alone was also informative ($r=0.833$), while LV systolic pressure alone was negatively correlated ($r=-0.801$) along this RV-loading path. On the RV side, the adjacent-pressure longitudinal-strain index correlated with RV stress-strain work density at $r=0.965$; peak RV longitudinal shortening alone did not ($r=0.090$), the simulated RV ejection fraction from the FEM cavity volumes correlated negatively ($r=-0.716$) reflecting that RV ejection falls as afterload rises while RV tissue work rises, and RV systolic pressure alone correlated strongly ($r=0.908$). Using LV pressure with RV strain was similarly high ($r=0.974$), because achieved LV and RV pressures are not independent along this capped-reference path. The important result is therefore not that one pressure is uniquely identified by this correlation test. It is that adding the adjacent pressure scale to longitudinal strain gives a strong stress-strain work-density ranking in both walls, with the clearest added value on the RV side.
...
A through-wall layer diagnostic gives a complementary view of the same issue. The geometric septum was split into LV-side, middle, and RV-side thirds using the LV-to-RV scalar used for the through-wall pressure diagnostics. This is not a fully resolved transmural stress measurement; the production mesh is only a few elements thick across the septum and is not designed to resolve detailed transmural layers. What it does show is that the septum is not internally uniform. The RV-side third generally carries larger stress-strain work density than the LV-side or middle thirds, with the asymmetry growing as RV pressure rises. The fibre and sheet-normal contributions also redistribute across layers across the sweep; {numref}`fig-septum-layer-mechanics` shows the per-layer pattern. This supports the conservative interpretation: a single scalar septal pressure-strain loop compresses a spatially heterogeneous tissue response into one number, and the through-wall weighted pressure introduced earlier is partly a way to track that heterogeneity.
...
|---|---|---|---|
| Primary capped sweep audit | 16 capped-reference cases | All cases completed per-case canonical quadrature-level postprocessing with finite pressure and per-cell work arrays | The primary figures and tables use a complete internally consistent regional result set |
...
The primary capped-reference sweep is complete: all 16 cases have solver pressure histories, metrics files, per-case canonical per-cell work data, and finite numeric arrays. The 5 mm capped jobs are separate tetrahedralizations of the same atlas target, with stored cell counts ranging from 7936 to 8109 cells; the analysis therefore uses volume-normalized regional integrals rather than one-to-one cell tracking across cases. The capped production set was not rerun at 10 mm, so the earlier h=10-to-h=5 comparison is treated as numerical-method support rather than as a direct convergence proof for the capped reference-state choice. Against the 3.75 mm mesh-convergence endpoints in the related production setup, peak pressures and end-diastolic volumes differed by less than 0.8% at 5 mm, and free-wall work-density ratios differed by less than about 3%. Septal quantities were more sensitive, especially at high RV pressure: the observed difference between the 5 mm and 3.75 mm meshes was about 5-7% for severe-case septal stress-strain work and septal longitudinal-proxy quantities. The completed capped cases also have geometric septum volumes within about 1.4% of the tag-3 septum volumes, so the primary septal region is not a non-canonical mask artifact.
