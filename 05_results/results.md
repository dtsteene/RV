...
Unless stated otherwise, the results use the high-resolution capped-reference pressure-loading sweep defined in {ref}`chap-calibration`: sixteen cases on a shared biventricular reference mesh ($n=8070$ cells), with the geometry, material model, fibre architecture, and end-diastolic mesh target held fixed and only the 0D circulation varied to raise RV pressure across the cases. The RV end-diastolic pressure used during inverse unloading is capped at 5 mmHg as motivated in {ref}`chap-calibration`. The sixteen cases also share the same region tag set (1269 geometric septum cells in every case; see {ref}`sec-reference-tag-postprocessing`), so regional integrals are case-comparable cell-by-cell. Reported pressures, correlations, and proxies use the cavity Lagrange-multiplier pressure from the coupled mechanics solve.
...
Two ratio errors are used. For the free-wall LV/RV comparison the reported error is the absolute difference $|R_\text{proxy}-R_\text{FE}|$ — this ratio sits consistently above one across the sweep (LV stress-strain work density exceeds RV throughout), so positive and negative deviations carry the same meaning and absolute difference is unambiguous. For the septum, two ratios are tested in each case (septum/LV-free-wall and septum/RV-free-wall) and they sit on opposite sides of unity — septum below the LV free wall, septum above the thinner RV free wall — so a symmetric metric is needed to count "twice too high" and "twice too low" equally. The reported error is therefore a mean absolute log ratio error,
...
Across the primary sweep, the longitudinal-strain proxy using each wall's adjacent cavity pressure correlated strongly with finite-element stress-strain work density in both walls ($r=0.994$ for the LV with $p_\text{LV}$, $r=0.967$ for the RV with $p_\text{RV}$). The single-component comparators in {numref}`tab-freewall-correlations` show why combining pressure and strain matters: in the LV, neither $p_\text{LV}$ nor longitudinal strain alone ranks the sweep usefully — $p_\text{LV}$ is nearly constant and falls the wrong way as the RV expands; in the RV, $p_\text{RV}$ alone carries most of the ranking signal but longitudinal shortening alone carries none. Replacing $p_\text{RV}$ with $p_\text{LV}$ on the RV side gives an even higher correlation ($r=0.992$), reflecting that achieved $p_\text{LV}$ and $p_\text{RV}$ co-vary along this capped-reference path; correlation alone therefore cannot pick between adjacent and non-adjacent pressure on the RV side. The LV/RV magnitude analysis above already rules out $p_\text{LV}$ on the RV side, however: it inflates the RV proxy and collapses the LV/RV ratio. The joint verdict is that wall-adjacent pressure paired with longitudinal strain is both correlation-strong and magnitude-stable, with the clearest added value on the RV side.
...
The septum is not a free wall. It is a shared internal wall with LV pressure on one side and RV pressure on the other. That makes the pressure assignment qualitatively different. A free wall has one comparatively unambiguous pressure scale; the septum has six plausible ones. Four are uniform across the septal volume: $p_\text{LV}$ (the usual clinical convention), $p_\text{RV}$ (the pressure on the opposite septal face), the transmural difference $p_\text{LV}-p_\text{RV}$ (the net pressure across the wall), and the mean $(p_\text{LV}+p_\text{RV})/2$ (the recognition that septal tissue is loaded from both sides). Two more vary across the septum's thickness: a nearest-side pressure that assigns $p_\text{LV}$ to cells on the LV side of the septum and $p_\text{RV}$ to cells on the RV side, and a through-wall weighted pressure that interpolates smoothly between $p_\text{LV}$ and $p_\text{RV}$ using a transventricular coordinate (formal definitions in {ref}`sec-work-definitions`).
...
The structural reason for this behaviour is shown in {numref}`tab-principal-strain-alignment`. The saved displacement checkpoints from the primary capped-reference sweep were replayed, the cell-centred Green--Lagrange strain tensor was diagonalised at each cell, and the most-compressive eigenvector was compared with the model fibre, longitudinal, radial, and circumferential directions in all three regions. Fibre is the best-aligned candidate in every region, with mean angles of 30 to 32 degrees and best-alignment in roughly 60% of the regional volume. Longitudinal alignment is markedly weaker — about 48 deg in the LV free wall, 54 deg in the RV free wall, and 50 deg in the septum — but it is not noticeably worse in the septum than in the free walls. The longitudinal-strain proxy's success in the free walls is therefore not explained by tighter strain-direction alignment there: the free-wall proxy works because adjacent cavity pressure consistently scales with the FE work that drives the cavity, while the septum-specific failure (transmural pressure especially) is a pressure-assignment problem rather than a strain-direction one. The fibre-aligned diagnostic above is consequently not just a rhetorical alternative; it follows from the deformation geometry of all three regions in this sweep.
...
Substituting the most-compressive principal-shortening direction at each point — which requires the full local strain tensor — also failed to identify a single best pressure: mean pressure was strongest for case ranking ($r=0.83$) while RV pressure preserved the septum/free-wall ratio best ($\eta_\text{ratio}=0.11$), and neither matched the fibre-aligned result where two-sided pressures dominate both tests. Because principal shortening is the most-compressive direction at each cell by construction, this also bounds what any other single-direction strain scalar — including a fitted linear combination of anatomical-axis strains with globally-fixed weights — can achieve on the same cells. Improvements beyond this point therefore require either a spatially-varying strain direction (the fibre-aligned diagnostic above demonstrates this within the model) or a proxy that uses shear strain components. The exercise reinforces that resolving the septal proxy needs three-dimensional strain information — either the full local tensor (shear components included) or, as a clinical compromise, a scalar combination of multiple anatomical-axis strains (longitudinal, circumferential, and radial) — rather than a different single scalar component.
