...
In the LV and RV free walls, adjacent cavity pressure is a reasonable regional stress scale: it preserves work-density ratios and tracks the sweep trend. The septum is different. As shared tissue between two pressurized cavities it cannot be treated as either free wall, and transmural pressure is not a sufficient work-density pressure scale — in the primary sweep it was the only pressure choice with a negative ranking correlation and the worst magnitude candidate. LV pressure gave the best fixed-candidate longitudinal magnitude result; the fibre-aligned diagnostic made mean and through-wall weighted pressure the most faithful choices.

This remains a model-side interpretation rather than a clinical replacement, because routine scalar strain components do not reconstruct the full strain tensor. The sweep correlations themselves are conditional on the achieved hemodynamic path, the chosen metric, and the numerical definition of the septal region — useful for sensitivity testing, not as universal clinical rankings.
