...
## Remaining Numerical Sensitivities

The main remaining postprocessing sensitivity is the choice of state/storage space used when reconstructing stress and strain histories. The final method uses degree-six quadrature storage because low-order projected stress-strain fields produced unacceptable energy-budget errors during development. A compact defence-oriented check is to recompute representative cases with deliberately degraded postprocessing variants and compare the resulting regional work densities against the final Quadrature6 pipeline.

The most useful cases for this check are sPAP22, sPAP60, and sPAP95 at the production 5 mm mesh, with an optional repeat of sPAP95 on the 3.75 mm mesh. This directly tests whether the final pressure-choice conclusions depend on the quadrature-space postprocessing decision.
