...
The updated check uses the same capped inverse-unloading convention as the production sweep, with the cell tags fixed once on the reference mesh inside each patient geometry. The healthy and PAH meshes have different anatomy and therefore cannot share a cell-by-cell tag set, so the table is still not a controlled case-by-case validation. Its value is directional: it asks whether a remodelled, thicker PAH geometry changes the relation between pressure-strain proxy values and finite-element stress-strain work density.
