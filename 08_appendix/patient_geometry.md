...
The updated check uses the same capped inverse-unloading convention as the production sweep and fixed shared-mask tags within each patient geometry. The healthy and PAH meshes cannot share a cell-by-cell shared-mask tag set because their anatomy is different, so the table is still not a controlled case-by-case validation. Its value is directional: it asks whether a remodelled, thicker PAH geometry changes the relation between pressure-strain proxy values and finite-element stress-strain work density.
