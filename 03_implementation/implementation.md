...
Pre-fix, peak fibre stresses sat systematically below the literature envelope {cite}`delhaas1994regional,finsberg2017phd`. The source was the compressible-formulation code path of `fenicsx-pulse` passing only the deviatoric part of the strain state to the stress evaluation, dropping the volumetric contribution that the Holzapfel-Ogden law requires. With the library patched to forward the full strain state, peak Cauchy fibre stress $|\sigma_{ff}|$ on the lowest-pressure case is 62, 50, and 41 kPa for the LV free wall, RV free wall, and septum — inside the literature envelope ({numref}`fig-stress-magnitudes`). A magnitudes check is necessary but not sufficient: a stress field can sit in the right band and still be wrong in its spatial distribution. The deviatoric bug was in fact one of three coincident errors that the energy-balance closure in the next section exposed; the magnitudes figure is the post-fix sanity confirmation, not the diagnostic that found the bug.
...
The three preceding sections each removed a different postprocessing or stress-evaluation error. The diagnostic that made them visible is whole-heart energy consistency: internal stress-strain work, accumulated through the cycle, equals cavity-pressure work plus Robin support work, by the divergence-theorem identity of {ref}`sec-energy-identity`. The cumulative closure residual is

$$
R(t) = W_\text{int}(t) - W_\text{cav}(t) - W_\text{Robin}(t),
$$

with $W_\text{int}(t) = \sum_i \int_\Omega \bar{\mathbf{S}}_i : \Delta\mathbf{E}_i \, dV_0$ accumulated to time $t$ (using the timestep-average second Piola-Kirchhoff stress $\bar{\mathbf{S}}_i$ and the Green-Lagrange strain increment $\Delta\mathbf{E}_i$), $W_\text{cav}(t) = \sum_i \bar p_i \, \Delta\mathcal{V}_i$ the cumulative cavity work, and $W_\text{Robin}(t)$ the cumulative Robin support work. Energy consistency requires $|R(T)|/|W_\text{cav}(T)| \ll 1$ at the cycle end $T$. Chapter 1 confirmed that this implementation achieves $\sim 10^{-5}$ on the UKB baseline ({numref}`fig-energy-balance`).
