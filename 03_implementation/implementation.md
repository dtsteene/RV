...
A second forward-problem check is whether peak fibre stress falls in the reported myocardial envelope {cite}`delhaas1994regional,finsberg2017phd`. On the lowest-pressure case of the production sweep, the per-region time-maximum of the cell-mean Cauchy fibre stress $|\sigma_{ff}|$ over the last beat is 62, 50, and 41 kPa for the LV free wall, RV free wall, and septum, inside the envelope ({numref}`fig-stress-magnitudes`).

```{figure} ../figures/fig_stress_magnitudes.png
:name: fig-stress-magnitudes
:width: 75%

Peak Cauchy fibre stress $|\sigma_{ff}|$ per region in the lowest-pressure case of the production sweep (peak LV pressure 105 mmHg, peak RV pressure 32 mmHg, bars) against a reported myocardial fibre-stress envelope (band, 20–80 kPa) drawn to bracket the canine epicardial peaks reported by Delhaas et al. {cite}`delhaas1994regional` (mean 21–27 kPa across regions; up to 2–3× higher transmurally) and the human computational estimates of Finsberg {cite}`finsberg2017phd`. The simulated peaks fall inside this envelope.
```

The remaining checks shift from the forward problem to postprocessing. The regional work-density numbers reported in {ref}`chap-results` come from an offline replay of the saved stress and strain fields, and three choices in that replay must agree with what the solver assembled: which cells define each region, how the boundary-work terms are formulated, and which cavity pressure record is read. The boundary and pressure choices were each wrong at some point during development; the region tagging was always self-consistent but matters for the cross-case comparison in {ref}`chap-results`. The whole-heart energy-consistency audit at the end of this chapter shows how the two boundary fixes close the budget.

(sec-shared-mask-tagging)=
## Region Definitions

The regional integrals require a definition of each region $\Omega_j$, and that definition must be stable across the loading sweep. The cell tags for the LV free wall, RV free wall, and septum are computed once on the imaged end-diastolic mesh and reused for every simulation in the sweep, rather than recomputed from each simulation's deformed end-diastolic state.
...
The diagnostic that makes the boundary-postprocessing errors visible is whole-heart energy consistency: internal stress-strain work, accumulated through the cycle, equals cavity-pressure work plus Robin support work, by time-integrating the rate identity derived in {ref}`chap-appendix-energy-identity` under the quasi-static equilibrium assumption that the solver enforces. The internal stress-strain work is accumulated to time $t$ as
...
and energy consistency requires $|R(T)|/|W_\text{cav}(T)| \ll 1$ at the cycle end $T$. The UKB baseline shown in {numref}`fig-energy-balance` of {ref}`chap-work-question` reaches $\sim 10^{-5}$ in this implementation; the production sweep here is no worse.

Development on this pipeline first put $|R(T)|/|W_\text{cav}(T)|$ near $0.33$, and the first-pass diagnosis blamed DG1 projection oscillations in the thin septum. Replay tests on the corrected pipeline show DG1 reproduces Quadrature6 integrated regional stress-strain work to within 1.2% (the largest gap is septum-localised, maximum across three representative sweep cases; the full DG0/DG1/Quadrature6 sensitivity is in {ref}`chap-appendix-numerical`), so the projection space was not the dominant whole-heart closure error. The two boundary postprocessors are what matter for the whole-heart budget on the corrected forward simulation. {numref}`fig-closure-audit` quantifies their contribution by replaying the production displacement under three postprocessing variants: the buggy boundary forms together (cavity-work read from 0D pressure, Robin work assembled with the reference-configuration form), with only the cavity-pressure fix applied, and the production configuration with both fixes.

```{figure} ../figures/fig_closure_audit.png
:name: fig-closure-audit
:width: 70%

Cycle-end closure residual $|R(T)|$ on the lowest-pressure sweep case (peak RV systolic 32 mmHg) as the two boundary fixes are applied successively, log scale. Each bar replays the saved production displacement under one postprocessing variant; the leftmost bar is not the historical dev-era state — it is the production displacement re-read with both buggy boundary forms. The relative residual $|R(T)|/|W_\text{cav}(T)|$ is shown in parentheses below each value; reduction factors between adjacent bars sit above the arrows.
```

The cavity-pressure record fix gives the larger absolute drop, cutting the residual from 21 mJ to 0.75 mJ; the contribution is concentrated in the volume-changing phases of the cycle, where the 0D-vs-Lagrange-multiplier pressure gap multiplies a non-zero $\Delta\mathcal{V}_i$. The Robin-formulation fix gives the larger fold reduction at $126\times$, from 0.75 mJ to 6 µJ, by removing the spurious tangential-sliding contribution that the reference-configuration form counted. The audit measures the *postprocessing* sensitivity of the closure budget on the corrected forward simulation; it is not a reproduction of the development-era 33% residual, which sat on a pipeline state that no longer exists.
...
The simulation driver, postprocessing pipeline, and figure-generation scripts used in this thesis are available at <https://github.com/dtsteene/cardiac-work>. The repository state corresponding to the results in this thesis is tagged as `thesis-v1`.
