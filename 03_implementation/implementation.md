...
A sensitivity check identified one limitation of using the same UKB baseline geometry and the same passive material parameters at every loading level of the sweep. At high RV end-diastolic pressures the unloading algorithm has to explain the added load with neither RV hypertrophy, curvature change, nor passive remodelling, and an uncapped pilot consequently inferred very small RV unloaded cavities for the high-RV-pressure cases — well outside the range the rest of the model assumes for the inferred unloaded geometry. The production sweep therefore caps the RV end-diastolic pressure used during inverse unloading at 5 mmHg, within the typical resting RV end-diastolic range of about 0–8 mmHg in healthy adults. The value was chosen empirically: lower caps were unnecessary because the low-RV-pressure cases produce moderate unloaded references without intervention, while higher caps reopened the small-unloaded-reference problem on the high-RV-pressure cases.

The concrete check that motivates this cap is round-trip consistency. Forward-simulating from the inferred unloaded reference back to end diastole should reproduce the imaged ED shape, since recovering that geometry is the inverse problem's stated objective. In the most severe uncapped case (sPAP95) this round-trip fails: the forward-simulated ED differs from the imaged ED by 9.4 mm whole-mesh mean and 13.9 mm at the septum (maximum local discrepancy 32 mm), while cavity volume is recovered exactly through the Lagrange-multiplier coupling. With the cap applied to the same case the residual drops to 3.7 mm whole-mesh and 4.3 mm at the septum, below typical CMR in-plane voxel resolution. The discrepancy is septum-localised: in the uncapped case the septum residual is about 1.5 times the LV-free-wall (9.0 mm) and RV-free-wall (8.2 mm) means, consistent with the septum being the region whose deformation mode is most sensitive to the unloaded reference. The cap therefore restores algorithmic consistency between the inverse-unloading and forward-inflation problems; without it the inverse solver and the forward solver disagree on what end-diastolic geometry to occupy, even though both honour the targeted cavity volume.

With the cap applied, the inferred unloaded cavity reductions across the 16 production sweep cases fall in the range 20–26% for the LV and 32–39% for the RV; the raw uncapped RV end-diastolic pressures targeted by these cases ranged from 4.8 to 16.3 mmHg, so the cap activated only for the higher-pressure cases and left the low-pressure cases unchanged. The cap is a bounded fixed-geometry correction, not a model of remodelling, and it does not propagate into the proxy comparison: the pressure-strain bookkeeping is anchored at start-of-beat strain and accumulates increments, which are invariant to translation of the reference state.
