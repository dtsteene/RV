...
The defining check on this procedure is round-trip consistency: forward-simulating from the inferred unloaded reference should reproduce the imaged end-diastolic shape, since that is the inverse problem's stated objective. A pilot sweep without intervention failed this check on the high-pressure cases. The most severe (sPAP95) missed the imaged end-diastole by 9.4 mm whole-mesh mean and 13.9 mm at the septum, with a peak local discrepancy of 32 mm, even though cavity volume was recovered exactly through the Lagrange-multiplier coupling — a failure that left the pressure-volume loop looking normal. The cause is structural: the baseline mesh and the passive material parameters are held fixed across every pressure level, so the algorithm cannot absorb a high RV end-diastolic pressure by hypertrophy, septal flattening, or stiffening. Its only remaining knob is the unloaded cavity volume, which it shrinks to unphysically small sizes at high RV pressures.

The production sweep avoids this by capping the RV end-diastolic pressure used during inverse unloading at 5 mmHg, within the typical resting right-heart filling-pressure range of 0–8 mmHg in healthy adults {cite}`humbert2022esc`. The cap acts only on the unloading step; the cycle solve still inflates to each case's full ED-pressure target. With the cap applied, the sPAP95 round-trip residual drops to 3.7 mm whole-mesh and 4.3 mm at the septum, below the in-plane CMR voxel size. The error is septum-localised, about 1.5 times the free-wall residuals, consistent with the septum being the region most sensitive to where the unloaded reference sits. The 5 mmHg value was chosen empirically: lower caps were unnecessary on the low-pressure cases, and higher caps reintroduced the small-cavity pathology on the high-pressure cases.
...
The septum is defined by the geometric distance rule in {ref}`sec-geometry-anatomical-model`, not by thresholding the LV-to-RV Laplace scalar. This is deliberate. The Laplace scalar is useful as a smooth coordinate: it can blend fibre rules and weight LV versus RV pressure across the septum. As a region label, however, it gives a less transparent knob: narrowing a scalar band does not simply produce a smaller anatomical septum, especially near the basal RV/LV junction. The geometric mask sweep in {ref}`sec-app-septum-epi-envelope` therefore perturbs the distance rule instead.
