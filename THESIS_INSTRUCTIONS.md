...
**Update, 2026-04-25:** the older framing in this file treated transmural septal pressure as the expected main result. That has been superseded by the newer analysis in `05_results/pressure_strain_framing_notes.md`, `05_results/results.md`, and `06_discussion/discussion.md`. The current framing is: adjacent pressure works reasonably well for LV/RV free-wall work ratios; the septum behaves as shared-wall tissue loaded by both cavities; two-sided septal pressure choices preserve septum/free-wall magnitudes better than transmural pressure; and pressure-sweep correlations are loading-path sensitivity results, not clinical PAH progression.
...
The central scientific question is deceptively simple: when a pressure-strain proxy uses a chamber pressure to stand in for the local myocardial stress tensor, where does that approximation work and where does it break? The current result is more nuanced than the older version of this file. Adjacent cavity pressure works reasonably well for the LV and RV free walls, where each wall is loaded mainly by one chamber. The septum is harder because it is shared tissue loaded by both cavities. Transmural pressure, meaning the difference between left and right ventricular pressure, is meaningful for septal force balance and shape, but it is not the best magnitude-preserving septal work proxy in the present analyses. Two-sided pressure choices such as mean or through-wall weighted pressure are more stable for septum/free-wall magnitude ratios.
...
**Key content to include:** The clinical relevance of RV dysfunction in PAH. The anatomical argument for why the septum complicates standard RV work estimation (it is a shared wall loaded by LV pressure from one side and RV pressure from the other). A brief preview of the computational approach. A one-sentence summary of the current main result: adjacent pressure works for free-wall ratios, while septal magnitude estimates need a two-sided pressure scale and pressure-sweep correlations are loading-path sensitive.
...
**Mathematical content:** The quantity being compared is the mechanical work done by the septal myocardium, computed as $W_\text{sep} = \int_\Omega \mathbf{S} : \dot{\mathbf{E}} \, dV$ where $\mathbf{S}$ is the second Piola-Kirchhoff stress tensor and $\mathbf{E}$ is the Green-Lagrange strain. This integral over the septal domain gives the finite-element tensor work. The clinical proxy is the area enclosed in a pressure-longitudinal-strain loop, $W_\text{PS,ll} \approx \oint P \, d\epsilon_\text{ll}$, because GLS-based speckle tracking measures longitudinal strain rather than fibre strain. A fibre-strain version, $W_\text{PS,ff} \approx \oint P \, d\epsilon_\text{ff}$, can still be used as a model-side comparator. The question is: which $P$ do you use for the septum, and does it matter?
...
The **active stress formulation** (`active_contaction.md`) needs expansion. The existing draft describes $\mathbf{S}_\text{total} = \mathbf{S}_\text{passive} + T_a (\mathbf{f}_0 \otimes \mathbf{f}_0)$ but does not explain the calcium-driven dynamics of $T_a(t)$. In the code, the Blanco ventricular activation model is used: the active tension rises from zero at end-diastole, peaks around 250 ms into systole, then decays during the relaxation phase. The total beat duration is calibrated to 75 beats per minute, giving a cycle length of 0.8 seconds. The LV free wall, RV free wall, and septum all use the same normalized waveform and a common peak active tension of 100 kPa.

**The region-aware activation storage** is one of the more subtle aspects of the implementation. Three distinct activation zones are defined — the LV free wall, the septum, and the RV free wall — so the infrastructure could assign different amplitudes if needed. In the simulations reported here, however, all three zones receive the same 100 kPa peak tension. The region tags remain important for work-density integration and diagnostics.

**Material parameters:** the simulations use `pulse.HolzapfelOgden.transversely_isotropic_parameters()`: $a = 2.280$ kPa, $b = 9.726$, $a_f = 1.685$ kPa, $b_f = 15.779$, with $a_s = b_s = a_{fs} = b_{fs} = 0$. The compressibility model is `Compressible2`, with bulk modulus $\kappa = 1000$ kPa and volumetric energy $\kappa (J^2 - 1 - 2\ln J)/4$.
...
This chapter should now be organized around the UKB synthetic baseline and the corrected pressure-loading sweep. Patient-specific anatomies are future-work/extension material unless their simulations are explicitly added to the current result set.

#### Hemodynamics Validation

Before examining work estimates, the simulated hemodynamics must be shown to be physiologically realistic. For the corrected sweep, this means showing that LV pressure is preserved while RV pressure increases along the intended loading path. Avoid presenting the sweep as a patient disease trajectory.

#### Work Decomposition

The internal elastic work of the septum is decomposed into fiber, cross-fiber (sheet), and wall-normal components. Results show that fiber shortening contributes approximately 35% of the total internal work in the LV, with the sheet direction contributing around 45% and the wall-normal direction accounting for the remainder. For the RV, the fiber fraction is higher at around 53%. This decomposition reveals that standard clinical work proxies, which implicitly assume that all myocardial work comes from fiber shortening, miss a substantial fraction of the tensor internal work.

#### Proxy Comparison

The current central result is a two-part comparison. First, ask whether the pressure-strain proxy preserves LV/RV free-wall work-density ratios within one simulation. Adjacent pressure does best here, which is the cleanest validation of the pressure-strain idea. Second, ask what happens in the septum. For the septum, compare LV pressure, RV pressure, transmural pressure, mean pressure, and through-wall weighted pressure. Report both correlations across the loading sweep and septum/free-wall ratio errors. The safest conclusion is not that one pressure is universally best. It is that correlations across a sweep are path-sensitive, while two-sided pressures are more stable for preserving septal work magnitudes.

**This is the thesis's key result.** Clinical septal pressure-strain work in RV pressure overload should be interpreted carefully because the pressure assigned to the septum is not a harmless convention. The present model supports adjacent pressure for free walls and two-sided pressure thinking for the septum; it does not prove a final clinical septal formula.

#### Energy Balance

This older note is superseded by the current energy-balance validation figure and the Results/Discussion framing. Do not reuse the obsolete claim that the tensor stress-strain integral returns about one-third of the cavity work across three cases. The current energy-balance point is narrower and cleaner: the whole-domain tensor work closes against the boundary work plus the Robin contribution to numerical precision in the validated UKB run, while regional pressure-strain proxies remain approximations because no exact regional PV identity exists.
...
**On the clinical implication:** The current result is more cautious than the older transmural-pressure framing. Myocardial work imaging — using speckle-tracking echocardiography to estimate strain and combining this with noninvasive pressure estimates — is increasingly used in clinical cardiology. Current implementations typically use one pressure convention for septal segments. The newer analysis suggests that free-wall work ratios are reasonably well captured by adjacent pressure, while septal work magnitudes are more stable when the septum is treated as shared tissue loaded by both cavities. This should be presented as a modelling insight and a motivation for clinical validation, not as a finished clinical correction.
