...
With the cell tags fixed on the reference mesh ({ref}`sec-shared-mask-tagging`), $\lambda$ is the saved LV-to-RV Laplace scalar — a smooth function from 1 on the LV side to 0 on the RV side, visualized in {numref}`fig-lv-rv-partition`.
